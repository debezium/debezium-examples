...
from .connect_message import ConnectMessageExtractor
...
class DebeziumRecord:
    ...
    @classmethod
    def from_source_record(cls, source_record) -> "DebeziumRecord":
        """
        ...
        extractor = SourceRecordExtractor(source_record)
        return cls(
            destination=extractor.destination,
            op=extractor.op or "r",
            before=extractor.before,
...
def connect_records_from_batch(source_records: List[Any]) -> List[DebeziumRecord]:
    """
    ...
    Returns:
        List of DebeziumRecord instances.
    """
    return [DebeziumRecord.from_source_record(r) for r in source_records]

...
docker compose --env-file ../.env -f docker-compose-mongodb-replicas.yaml exec kafka /kafka/bin/kafka-console-consumer.sh --bootstrap-server kafka:9092 --from-beginning --property print.key=true --topic dbserver1.inventory.customers
```
...
docker compose --env-file ../.env -f docker-compose-mongodb-replicas.yaml exec mongodb3 mongosh "mongodb://mongodb1:27017,mongodb2:27017,mongodb3:27017/inventory?replicaSet=rs0&authSource=admin" -u debezium -p dbz --authenticationDatabase admin
...
```
docker compose --env-file ../.env -f docker-compose-mongodb-replicas.yaml stop mongodb1
```

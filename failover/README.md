...
docker-compose -f docker-compose-mysql.yaml exec mysql1 bash -c 'mysql -u root -p$MYSQL_ROOT_PASSWORD inventory'
...
docker-compose -f docker-compose-mysql.yaml exec mysql2 bash -c 'mysql -u root -p$MYSQL_ROOT_PASSWORD inventory'
  SHOW GLOBAL VARIABLES LIKE 'server_uuid';
...
```
docker-compose -f docker-compose-mysql.yaml exec kafka /kafka/bin/kafka-console-consumer.sh --bootstrap-server kafka:9092 --from-beginning --property print.key=true --topic my_connect_offsets
